...
RegisterNUICallback('importVehicle', function(data, cb)
	if cooldown == nil then
		...
		if truck then
			...
			Framework.Functions.Notify("Bạn đã có một giao hàng đang được tiến hành", "error", 5000)
			...
			if checkPos == false then
				...
				Framework.Functions.Notify("Nhà để xe bị chặn", "error", 5000)
...
AddEventHandler('lc_dealership:startContract', function(vehicle,amount,isexport,request_id)
	...
	Framework.Functions.Notify("Đã lấy xe nhập hàng thành công, hãy lên xe và gắn thùng hàng để bắt đầu", "success", 7000)
	...
	while truck do
		...
		if IsEntityDead(ped) then
			...
			Framework.Functions.Notify("Bạn đã chết và thất bại", "error", 7000)
		...
		if engineH <= 150 then
			...
			Framework.Functions.Notify("Phương tiện của bạn đã bị phá hủy", "error", 7000)
